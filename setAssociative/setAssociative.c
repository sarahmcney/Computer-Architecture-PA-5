...
void accessData (
    ...
    /* ... */
    mem_addr_t set = ((1 << s)-1) & (addr >> b);
    mem_addr_t tag = addr >> (s+b);
   // cache_set_line_t curr_set = cache[set];
    ...
    (*miss_count)++;
    for(int i = 0; i < E; i++) { //try to find an empty slot in the set
        if(cache[set][i].valid == false) {
            cache_line_t tmp;
            tmp.tag = tag;
            tmp.valid = true;
            tmp.recent_use = program_counter;
            cache[set][i] = tmp;
            return;
        }
    }
    //if theres no empty slot you need to evict
    //identify least recently used
    (*eviction_count)++;
    int lru_index = 0;
    int lru_num = cache[set][0].recent_use;
    for(int i = 1; i < E; i++) {
        if(cache[set][i].recent_use < lru_num) {
            lru_num = cache[set][i].recent_use;
            lru_index = i;
        }
    }
    //write to lru slot
